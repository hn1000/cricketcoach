...
const submit = () => {
    form.put(route('company.staff.update', props.staff.id));
};

const deleteStaff = () => {
    if (confirm('Are you sure you want to delete this coach? This action cannot be undone.')) {
        router.delete(route('company.staff.destroy', props.staff.id));
    }
};
</script>
...
<template>
    ...
    <CompanyLayout>
        <div class="mb-4">
            ...
            <h1 class="mt-2 text-2xl font-bold text-gray-900 dark:text-white">Edit Coach</h1>
        </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="flex justify-between items-center pt-4 border-t border-gray-200 dark:border-gray-700">
                    <button
                        type="button"
                        @click="deleteStaff"
                        class="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg focus:ring-4 focus:ring-red-300"
                    >
                        Delete Coach
                    </button>
