...
const deleteStaff = (id) => {
    if (confirm('Are you sure you want to delete this staff member?')) {
        router.delete(route('admin.companies.staff.destroy', [props.company.id, id]));
    }
};
</script>

<template>
    <Head :title="`Staff - ${company.name}`" />
    ...
    <AdminLayout>
        ...
        <div class="bg-white dark:bg-gray-800">
            <Tabs :company="company" active-tab="staff" />
        </div>
                    ...
                    <div class="overflow-hidden shadow">
                        <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                <tr v-for="member in staff.data" :key="member.id" class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white">
                                        <div class="font-semibold">{{ member.full_name }}</div>
                                        <div class="text-sm font-normal text-gray-500 dark:text-gray-400">{{ member.department || '-' }}</div>
                                    </td>
                                    ...
                                    <td class="p-4 space-x-2 whitespace-nowrap">
                                        <Link
                                            :href="route('admin.companies.staff.calendar.index', [company.id, member.id])"
                                            class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-purple-600 hover:bg-purple-700 focus:ring-4 focus:ring-purple-300 dark:bg-purple-500 dark:hover:bg-purple-600 dark:focus:ring-purple-800"
                                        >
                                        ...
                                        <Link
                                            :href="route('admin.companies.staff.availability.index', [company.id, member.id])"
                                        ...
                                        <Link
                                            :href="route('admin.companies.staff.edit', [company.id, member.id])"
