...
const submit = () => {
    form.post(route('admin.companies.staff.exceptions.store', [props.company.id, props.staff.id]));
};
</script>

<template>
    <Head :title="`Add Exception - ${staff.first_name} ${staff.last_name}`" />
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-2">
                        ...
                        <li aria-current="page">
                            <div class="flex items-center">
                                ...
                                <span class="ml-1 text-gray-500 md:ml-2 dark:text-gray-400">Add Exception</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Add Availability Exception</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Block off a specific date or time when the staff member is unavailable (vacation, meeting, holiday, etc.)
                </p>
            </div>
        </div>
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                            Unavailable: Block time off. Custom Hours: Different availability for this day.
                        </p>
                    </div>
                ...
                <div v-if="isFullDayBlock" class="p-4 mb-4 bg-red-50 border border-red-200 rounded-lg dark:bg-red-900 dark:border-red-700">
                    <div class="flex items-start">
                        ...
                        <div class="text-sm text-red-700 dark:text-red-300">
                            ...
                            <p class="mt-1">No booking slots will be available for this date. To block only specific hours, enter start and end times.</p>
                        </div>
                    </div>
                </div>

                <div v-else-if="form.start_time && form.end_time" class="p-4 mb-4 bg-yellow-50 border border-yellow-200 rounded-lg dark:bg-yellow-900 dark:border-yellow-700">
                    <div class="flex items-start">
                        <svg class="w-5 h-5 text-yellow-700 dark:text-yellow-300 mr-2 mt-0.5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path>
                        </svg>
                        <div class="text-sm text-yellow-700 dark:text-yellow-300">
                            <p class="font-medium">Partial day block</p>
                            <p class="mt-1">Booking slots between {{ form.start_time }} and {{ form.end_time }} will be unavailable for this date.</p>
                        </div>
                    </div>
                </div>
